...
export async function search_for_item(query: string, country: string): Promise<any> {
    console.log(proxy());
    const url = `https://cors.wannaflix.net/https://apis.justwatch.com/content/titles/${country}/popular`
...
async function get_locales(): Promise<any> {
    const url = "https://cors.wannaflix.net/https://apis.justwatch.com/content/locales/state"
    const response = await fetch(url, {
...
async function get_providers(country: string): Promise<any> {
    const url = `https://cors.wannaflix.net/https://apis.justwatch.com/content/providers/locale/${country}`
    const response = await fetch(url, {
      headers: {"X-Requested-With": "fetch"}
    })
    if (response.ok == false) throw new Error("Http Error: " + response.status)
...
export async function get_title_from_id(title_id: string, country: string, content_type: string): Promise<any> {
    const url = `https://cors.wannaflix.net/https://apis.justwatch.com/content/titles/${content_type}/${title_id}/locale/${country}`
    const response = await fetch(url, {
      headers: {"X-Requested-With": "fetch"}
    })
    if (response.ok == false) throw new Error("Http Error: " + response.status)
